...
import { fetchPrescriptions, updatePrescriptionStatus, createPrescription } from "../api/prescriptionsApi";
...
export function usePrescriptions() {
  ...
  const filtered = useMemo(() => {
    ...
    return prescriptions.filter((p) => {
      const matchesSearch =
        q === "" || p.patient.toLowerCase().includes(q) || p.code.toLowerCase().includes(q);
